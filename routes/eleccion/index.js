...
router.post('/generadorIA/:nombreEscenario/:nombrePersonaje/:idTextoIA/:idParrafo', async function (req, res, next) {
  ...
  if (req.body.accion == "ingresar") {
    var numeroParrafoMasUno = Number(req.params.idParrafo.substring(7)) + Number(1);
    var idParrafo = "parrafo" + numeroParrafoMasUno;
    var nuevaAccion = req.body.accionTexto;
    
    if (numeroParrafoMasUno > Number(30)) {
      res.redirect('/ia/gameOver');
  ...
  if (idParrafo == "parrafo1") {
    ...
    //var accionParrafo = "You " + nuevaAccion;
  }

  console.log(numeroParrafoMasUno + ": " + parrafo);
...
router.get('/gameOver', function (req, res, next){
  ...
  res.locals.title = "IA";
  res.locals.content = "finish/gameOver";
...
router.get('/misHistorias', async function (req, res, next){

  let sessionIdUsuario = req.session.usuario.idUsuario;

  let filter = { where: { idUsuario: sessionIdUsuario, estado: 1 } };
  res.locals.textos = await Models.TextoIA.findAll(filter);

  res.locals.title = "IA";
  res.locals.tituloForm = "Mis Historias";
  res.locals.content = "texto/misHistorias";

  res.render('page');
});


router.get('/historia/:idTexto', async function (req, res, next){
  let filter = { where: { idTextoIA: req.params.idTexto } };
  res.locals.texto = await Models.TextoIA.findAll(filter);

  res.locals.title = "IA";
  res.locals.tituloForm = `Historia ${req.params.idTexto}`;
  res.locals.content = "historia/historia";
...
router.get('/consejos', function (req, res, next){
  ...
  res.locals.title = "IA";
  res.locals.tituloForm = "Consejos";
  res.locals.content = "historia/consejos";
...
router.get('/acercaDe', function (req, res, next){
  ...
  res.locals.title = "IA";
  res.locals.tituloForm = "Acerca De";
  res.locals.content = "historia/acercaDe";
...
router.get('/historia/delete/:idTextoIA', async function (req, res, next){
  
  res.locals.texto = { idTextoIA: req.params.idTextoIA };
  res.locals.content = 'historia/delete';
  res.locals.title = "IA";
  res.locals.tituloForm = "Eliminar Historia";
  res.locals.content = "historia/delete";

  res.locals.obj = await Models.TextoIA.findByPk(req.params.idTextoIA);

  res.render('page');
});

router.post("/historia/delete/:idTextoIA", async function (req, res, next) {

  let obj = await Models.TextoIA.findByPk(req.params.idTextoIA);
  obj.estado = 0;
  await obj.save();

  res.redirect("/ia/misHistorias");
});
